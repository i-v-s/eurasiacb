...
#define KEYCODE_YAW_L 0x61 // "a" character
#define KEYCODE_YAW_R 0x64 // "d" character
...
void TeleopKey::keyLoop()
{
    ...
    for(;;)
    {
        ...
        if(retval==-1)
            ...
            switch(c)
            {
                case KEYCODE_L: // left arrow
                    ROS_DEBUG("LEFT");
                    //ctrl_msg_.yaw > MIN_YAW ? ctrl_msg_.yaw-- : ctrl_msg_.yaw = MIN_YAW;
                    ctrl_msg_.roll > MIN_ROLL ? ctrl_msg_.roll-- : ctrl_msg_.roll = MIN_ROLL;
                    dirty = true;
                                    break;
                case KEYCODE_R: // right arrow
                    ROS_DEBUG("RIGHT");
                    //ctrl_msg_.yaw < MAX_YAW ? ctrl_msg_.yaw++ : ctrl_msg_.yaw = MAX_YAW;
                    ctrl_msg_.roll < MAX_ROLL ? ctrl_msg_.roll++ : ctrl_msg_.roll = MAX_ROLL;
                    dirty = true;
                break;
                case KEYCODE_YAW_L: // "a" character
                    ROS_DEBUG("YAW LEFT");
                    ctrl_msg_.yaw > MIN_YAW ? ctrl_msg_.yaw-- : ctrl_msg_.yaw = MIN_YAW;
                    dirty = true;
                break;
                case KEYCODE_YAW_R: // "b" character
                    ROS_DEBUG("YAW RIGHT");
                    ctrl_msg_.yaw < MAX_YAW ? ctrl_msg_.yaw++ : ctrl_msg_.yaw = MAX_YAW;
                    dirty = true;
                break;
                case KEYCODE_U: // up arrow
                    ROS_DEBUG("FORWARD");
                    ctrl_msg_.nick < MAX_NICK ? ctrl_msg_.nick++ : ctrl_msg_.nick = MAX_NICK;
                    dirty = true;
                break;
                case KEYCODE_D: // down arrow
                    ROS_DEBUG("BACK");
            ...
            if(dirty) {
                ROS_INFO("nick=%d, gas=%d, yaw=%d, roll=%d, fix=%s, on=%s",
                         ctrl_msg_.nick, ctrl_msg_.gas, ctrl_msg_.yaw, ctrl_msg_.roll,
            ...
            if(ctrl_msg_.roll!=DEF_ROLL) ctrl_msg_.roll > DEF_ROLL? ctrl_msg_.roll-- : ctrl_msg_.roll++;
